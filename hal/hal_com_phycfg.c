...
#ifdef CONFIG_LOAD_PHY_PARA_FROM_FILE
    if (hal_file_name) {
        rtw_merge_string(para_file_path, PATH_LENGTH_MAX, rtw_phy_file_path, hal_file_name);
        return rtw_is_file_readable(para_file_path);
    }
#endif
    return _FALSE;
}
...
)
{
    ...
    if ((pHalData->mac_reg_len == 0) && (pHalData->mac_reg == NULL))
    {
        ...
        {
            rlen = rtw_retrieve_from_file(para_file_path, pHalData->para_file_buf, MAX_PARA_FILE_BUF_LEN);
            if (rlen > 0)
            {
                rtStatus = _SUCCESS;
